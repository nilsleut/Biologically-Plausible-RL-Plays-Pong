...
def main():
    ...
    save_dir = Path(r"C:\Users\nilsl\Desktop\Projekte\Biological_plausible_distributed_rl\results\comparison")
    save_dir.mkdir(parents=True, exist_ok=True)
    
    # Shared environment config (easy opponent for faster learning)
    pong_config = PongConfig(
        opponent_skill=0.7,
        opponent_speed=0.03,
        opponent_reaction_delay=3,
        max_steps=500,
        reward_hit=0.1,
    )
    
    N_TIMESTEPS = 500000  # Use 500k+ locally
    
    print("=" * 60)
    print("PPO vs Hebbian on Pong")
    print("=" * 60)
    ...
    plt.suptitle(f"PPO vs Hebbian on Custom Pong ({N_TIMESTEPS:,} steps)", fontsize=14)
    plt.tight_layout()
    fig.savefig(save_dir / "01_ppo_vs_hebbian.png", bbox_inches="tight")
    plt.close()
    print("  Saved 01_ppo_vs_hebbian.png")
